...
import { parseJsonFile, retrieveTopicNameToPublish, ValidationError } from "../../utils/utility.js";
...
describe('Utility Functions', () => {
    ...
    it("Validates parseJsonFile function", async () => {
        var readJSONObject = parseJsonFile('__test__/sample_file.json');
    ...
    it("Validates retrieveTopicNameToPublish function for topic name without *", async () => {
        var retrievedTopicName =
            retrieveTopicNameToPublish('detection:channel:1', 'detection/channel/mqtt/1', 'detection:channel:1');
        expect(retrievedTopicName).toBe('detection/channel/mqtt/1');
    });
    ...
    it("Validates retrieveTopicNameToPublish function for topic name with *", async () => {
        var retrievedTopicName =
            retrieveTopicNameToPublish('detection:ppe:1', 'detection/ppe/mqtt/*', 'detection:ppe:*');
        expect(retrievedTopicName).toBe('detection/ppe/mqtt/1');
    });
});