...
import { MQTTPublisher } from "./utils/mqtt.js";
...
export class CloudConnect {
    ...
    constructor(configJSONFile) {
        ...
        this.#brokerObject[MQTTConstants.CONFIG_HOST_KEY] = process.env.BROKER_HOST;
        this.#brokerObject[MQTTConstants.CONFIG_PORT_KEY] = process.env.BROKER_PORT;
        ...
        if (typeof this.#brokerObject[MQTTConstants.CONFIG_HOST_KEY] == 'undefined'
            || String(this.#brokerObject[MQTTConstants.CONFIG_HOST_KEY]).trim() == '') {
            throw new ValidationError("Environment variable BROKER_HOST is not set.");
        }

        // Validate environment variable BROKER_HOST is set.
        if (typeof this.#brokerObject[MQTTConstants.CONFIG_PORT_KEY] == 'undefined'
            || String(this.#brokerObject[MQTTConstants.CONFIG_PORT_KEY]).trim() == '') {
    ...
    initialiseSubscribers() {
        ...
        // Parse the configuration file to get list of topic to subcribe along with
        ...
        for (var index = 0; index < redisConsumersArray.length; index++) {
            ...
            var clientIdPrefix = this.#configObject[MQTTConstants.CONNECTION_ID_PREFIX_KEY];
            var isSSLEnabled = Boolean(Number(this.#configObject[MQTTConstants.CONNECTION_ENABLE_SSL_KEY]));
            var sslConfig = this.#configObject[MQTTConstants.CONNECTION_SSL_CONFIG_KEY];
            ...
            var cloudClient = new MQTTPublisher(this.#brokerObject[MQTTConstants.CONFIG_HOST_KEY],
                this.#brokerObject[MQTTConstants.CONFIG_PORT_KEY], messageErrorhandleFlag,
                clientIdPrefix, isSSLEnabled, sslConfig);
            // Set the Client id so that it is available for the callback method of the instance
            var cloudClientId = cloudClient.getClient().options.clientId;
            cloudClient.setClientId(cloudClientId);
