...
FILE NAME
    cloud_connect_kafka_config.h

DESCRIPTION
    Header file for the configuration of Cloud Connect kafka service.
*/

#ifndef CLOUD_CONNECT_KAFKA_CONFIG_H_
#define CLOUD_CONNECT_KAFKA_CONFIG_H_
...
/**
 * @struct ssl_config
 * @brief Cloud connect ssl configuration structure
 */
typedef struct cck_ssl_config
{   
 ...
 */
typedef struct cck_channel_mapping
{
    int num_channels;   /*!< number of channels */
    char **channel_src; /*!< redis channel name or pattern to be subscribed */
    char **channel_dst; /*!< kafka channel name or pattern over which data to be published */
} cck_channel_mapping_t;

/**
 * @struct sys_config
 * @brief Cloud connect config structure
 */
typedef struct cck_sys_config
{
    /* Environment variable driven params*/
    char *redis_host;                       /*!< redis host name */
    int redis_port;                         /*!< redis port number */
    char *kafka_host;                       /*!< kafka server host name */
    int kafka_port;                         /*!< kafka port number */
    char *kafka_msg_max_bytes;              /*!< kafka message max bytes */
    ...
    cck_channel_mapping_t *channel_mapping; /*!< list of channel mapping */
    char *client_id_prefix;                 /*!< Prefix to be used for client id. Max 10 characters */
    bool enable_ssl_connection;             /*!< a boolean to indicate secure connection is enabled or not */
    cck_ssl_config_t ssl_config;              /*!< SSL configurations */
} cck_sys_config_t;

/**
 * @fn cloud_connect_kafka_init_config()
 * @brief Function to initialize the configuration of the cloud connect kafka microservice.
 *
 * @return int
...
/**
 * @fn cloud_connect_kafka_get_config()
 * @brief Function to get loaded configuration of the cloud connect kafka microservice.
 *
 * @return cck_sys_config_t *
 */
cck_sys_config_t *cloud_connect_kafka_get_config();

/**
 * @fn cloud_connect_kafka_deinit_config()
 * @brief Function to free the configuration of the cloud connect kafka microservice.
 *
 * @return void
 */
