...
FILE NAME
    cloud_connect_kafka_env_utils.c
...
const char *cloud_connect_kafka_get_env_value_str(const char *env_var, const char *default_value)
{
    const char *value = getenv(env_var);
    return (value != NULL && strlen(value) > 0) ? value : default_value;
}

int cloud_connect_kafka_get_env_value_int(const char *env_var, int default_value)
{
    const char *value = getenv(env_var);
