...
#define DEFAULT_MODEL_PATH    "/etc/models/foot_track_net_quantized.tflite"
#define DEFAULT_LABELS_PATH   "/etc/labels/foot_track_net.json"
#define DEFAULT_SETTINGS_PATH "/etc/labels/foot_track_net_settings.json"
#define DEFAULT_ZONE_BOUNDARIES \
  "Zones,Zone1=<<960,540>,<1920,540>,<1920,1080>,<960,1080>>;"
...
gst_app_config_apply_defaults (GstAppConfig * config)
{
  ...
  if (config->settings_path == NULL)
    config->settings_path = g_strdup (DEFAULT_SETTINGS_PATH);

  if (config->zone_boundaries == NULL)
    config->zone_boundaries = g_strdup (DEFAULT_ZONE_BOUNDARIES);
...
gst_app_config_free (GstAppConfig * config)
{
  ...
  g_free (config->settings_path);
  g_free (config->zone_boundaries);
...
/*
 * Convert a zone-config boundaries string to a qtivoverlay masks string.
 *
 * Input format:  "Zones,Zone1=<<x,y>,...>,Zone2=<<x,y>,...>;"
 * Output format: "{(structure)\"Zone1,polygon=<<x,y>,...>,color=0x7F00007F; ...\"}"
 *
 * The caller must g_free() the returned string.
 */
static gchar *
gst_app_zone_boundaries_to_overlay_masks (const gchar * zone_boundaries)
{
  gchar *result = NULL;
  gchar *body = NULL;
  gchar **zone_entries = NULL;
  GString *masks_str = NULL;
  gint i = 0;

  if (!zone_boundaries || zone_boundaries[0] == '\0')
    return g_strdup ("");

  /* Strip the "Zones," prefix */
  if (g_str_has_prefix (zone_boundaries, "Zones,"))
    body = g_strdup (zone_boundaries + strlen ("Zones,"));
  else
    body = g_strdup (zone_boundaries);

  /* Split on ";" to get individual zone entries */
  zone_entries = g_strsplit (body, ";", -1);
  g_free (body);

  masks_str = g_string_new ("{(structure)\"");

  for (i = 0; zone_entries[i] != NULL; i++) {
    const gchar *entry = zone_entries[i];

    if (entry[0] == '\0')
      continue;

    /* Split on the first "=" to separate zone name from polygon */
    ...
    g_string_append_printf (masks_str,
        "%s,polygon=%s,color=0x7F00007F;",
        g_strstrip (parts[0]),
        parts[1]);
  ...
  g_strfreev (zone_entries);
  g_string_append (masks_str, "\"}");

  result = g_string_free (masks_str, FALSE);
  return result;
}
    ...
    GstElement ** user_tail)
{
  ...
  GstElement *qtirestrictedzonedbg = NULL;
  ...
  if (!qtimlvconverter  || !qtimltflite   || !qtimlpostprocess ||
      ...
      (!appctx->config.no_debug && !qtirestrictedzonedbg)) {
  ...
  gchar *settings_path = g_strdup (appctx->config.settings_path);
  ...
  if (!appctx->config.no_debug) {
    g_object_set (G_OBJECT (qtirestrictedzonedbg),
        "zone-config", appctx->config.zone_boundaries,
        NULL);
  }
  ...
  if (!appctx->config.no_debug)
    gst_bin_add (GST_BIN (appctx->pipeline), qtirestrictedzonedbg);
  ...
  // Linking metamux through tracker, optional zone debug, to user_tail
  if (!appctx->config.no_debug) {
    gst_element_link_many (qtimetamux, queue[5], qtiobjtracker,
        qtirestrictedzonedbg, NULL);
    *user_tail = qtirestrictedzonedbg;
  } else {
    gst_element_link_many (qtimetamux, queue[5], qtiobjtracker, NULL);
...
main (int argc, char * argv[])
{
  ...
  GOptionEntry entries[] = {
    ...
    { "labels-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.labels_path,
      "Path to labels JSON file", "PATH" },
    { "settings-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.settings_path,
      "Path to model settings JSON file", "PATH" },
    { "restricted-zone-boundaries", 0, 0, G_OPTION_ARG_STRING, &appctx.config.zone_boundaries,
      "Zone boundaries string, e.g. \"Zones,Zone1=<<x,y>,...>;\" (auto-derives overlay polygon)", "ZONES" },
