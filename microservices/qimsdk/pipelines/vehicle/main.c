...
/*
 * Application: Programmatic GStreamer Vehicle Detection Usecase
 *
 * Description:
 *   This application demonstrates a real-time vehicle detection pipeline using
 *   GStreamer. It processes video streams to detect vehicles with a YOLO-family
 *   detection model on the NPU, assigns stable track ids to the detections, and
 *   draws bounding boxes and class labels directly in the video. The annotated
 *   video and metadata can be streamed over RTSP or WebRTC, and the metadata is
 *   published to Redis for the vehicle-analytics microservices.
 *
 ...
 *   --input-type=file  --input-config=/path/to/video.ts
 *
 * Supported outputs:
 *   --output-type=none
 ...
 *
 * Example usage:
 *   ./gst-vehicle-detection --input-type=isp --width=1280 --height=720
 *
 *   ./gst-vehicle-detection \
 *       --input-type=usb \
 ...
 *
 * Notes:
 *   - .ts file input uses tsdemux + h264parse + v4l2h264dec (hardware H.264).
 *   - .mp4 and RTSP inputs use qtiuridecodebin (URI-based auto-decode).
...
#define DEFAULT_MODEL_PATH  "/etc/models/yolox-yolo-x-w8a8.tflite"
#define DEFAULT_LABELS_PATH "/etc/labels/yolov8n.labels"
#define DEFAULT_CONFIDENCE  51.0
...
typedef struct GstAppContext
{
  ...
  GstAppPadLinkData decodebin_link;
...
/*
 * Handle tsdemux dynamic pads and link the first H.264 video stream.
 *
 * tsdemux exposes individual elementary streams from an MPEG-TS container.
 * This callback accepts only video/x-h264 and links it to h264parse.
 */
static void
gst_app_tsdemux_pad_added_cb (GstElement * tsdemux, GstPad * srcpad,
    gpointer userdata)
{
...
/*
 * Handle qtiuridecodebin dynamic pads.
 *
 * Links only the first video/x-raw stream to the downstream capsfilter.
 */
static void
gst_app_decodebin_pad_added_cb (GstElement * decodebin, GstPad * srcpad,
    gpointer userdata)
{
  GstAppPadLinkData *link_data = (GstAppPadLinkData *) userdata;
  GstCaps *caps = NULL;
  ...
  link_data->linked =
      gst_app_link_dynamic_src_pad (srcpad, link_data->target);
...
gst_app_config_apply_defaults (GstAppConfig * config)
{
  ...
  if (config->model_path == NULL)
    config->model_path = g_strdup (DEFAULT_MODEL_PATH);

  if (config->labels_path == NULL)
    config->labels_path = g_strdup (DEFAULT_LABELS_PATH);

  if (config->confidence <= 0.0)
    config->confidence = DEFAULT_CONFIDENCE;

  if (config->results <= 0)
    config->results = DEFAULT_RESULTS;

  if (config->webrtc_id <= 0)
    config->webrtc_id = DEFAULT_WEBRTC_ID;

  if (config->enable_redis_meta) {
    if (!config->redis_host)
      config->redis_host = g_strdup ("127.0.0.1");

    if (config->redis_port <= 0)
      config->redis_port = 6379;

    if (!config->redis_channel)
      config->redis_channel = g_strdup ("detections");
  }
}
...
gst_app_config_validate (const GstAppConfig * config)
{
  ...
  if (g_strcmp0 (config->input_type, "file") == 0 &&
      !g_str_has_suffix (config->input_config, ".mp4") &&
      !g_str_has_suffix (config->input_config, ".ts")) {
    g_printerr ("ERROR: --input-config for file input must end in .mp4 or .ts.\n");
    return FALSE;
  }
  ...
  if (config->confidence <= 0.0 || config->confidence > 100.0) {
    g_printerr ("ERROR: --confidence must be in the range (0, 100].\n");
    return FALSE;
  }
...
gst_app_config_free (GstAppConfig * config)
{
  ...
  g_free (config->model_path);
  g_free (config->labels_path);
  g_free (config->redis_host);
  g_free (config->redis_channel);
}
    ...
    */
    ...
    if (!ret) {
      ...
      goto error;
             ...
             g_strcmp0 (appctx->config.input_type, "rtsp") == 0) {
    gboolean is_ts = g_strcmp0 (appctx->config.input_type, "file") == 0 &&
        g_str_has_suffix (appctx->config.input_config, ".ts");

    if (is_ts) {
      /*
       * MPEG-TS file input: filesrc reads the .ts file, tsdemux demultiplexes the
       * transport stream, h264parse normalizes the elementary stream, and
       * v4l2h264dec decodes it on the hardware decoder. A capsfilter enforces
       * NV12 before the common input_queue handoff.
       */
      GstElement *filesrc = gst_app_make_element ("filesrc", "ts_filesrc");
      GstElement *tsdemux = gst_app_make_element ("tsdemux", "ts_demux");
      GstElement *parse   = gst_app_make_element ("h264parse", "ts_h264parse");
      GstElement *decoder = gst_app_create_h264_decoder ();
      GstElement *capsfilter_ts = gst_app_make_element ("capsfilter", "input_capsfilter");
      GstCaps *ts_caps = NULL;

      if (!filesrc || !tsdemux || !parse || !decoder || !capsfilter_ts)
        goto error;

      g_object_set (G_OBJECT (filesrc), "location", appctx->config.input_config, NULL);

      ts_caps = gst_caps_new_simple ("video/x-raw",
          "format", G_TYPE_STRING, "NV12",
          NULL);
      g_object_set (G_OBJECT (capsfilter_ts), "caps", ts_caps, NULL);
      gst_caps_unref (ts_caps);

      gst_bin_add_many (GST_BIN (appctx->pipeline),
          filesrc, tsdemux, parse, decoder, capsfilter_ts, NULL);

      /* Link static part: parse -> decoder -> capsfilter -> queue */
      if (!gst_element_link_many (parse, decoder, capsfilter_ts, queue, NULL)) {
        g_printerr ("ERROR: Failed to link TS decode chain.\n");
        goto error;
      }

      /* filesrc -> tsdemux is static; tsdemux -> parse uses a dynamic pad */
      if (!gst_element_link (filesrc, tsdemux)) {
        g_printerr ("ERROR: Failed to link filesrc to tsdemux.\n");
        goto error;
      }

      appctx->decodebin_link.target = parse;
      appctx->decodebin_link.linked = FALSE;

      g_signal_connect (tsdemux, "pad-added",
          G_CALLBACK (gst_app_tsdemux_pad_added_cb),
          &appctx->decodebin_link);
    } else {
      /*
       * MP4 file input or RTSP input: qtiuridecodebin handles URI-based
       * auto-decode. File paths are converted to file:// URIs; rtsp:// URLs
       * are passed as-is.
       */
      GstElement *decodebin = gst_app_make_element ("qtiuridecodebin", "uri_decode");
      gchar *uri = NULL;

      if (!decodebin || !queue)
        goto error;

      if (g_str_has_prefix (appctx->config.input_config, "rtsp://")) {
        uri = g_strdup (appctx->config.input_config);
      } else {
        uri = g_strdup_printf ("file://%s", appctx->config.input_config);
      }
      ...
      gst_bin_add_many (GST_BIN (appctx->pipeline),
          decodebin, capsfilter, queue, NULL);

      if (!gst_element_link_many (capsfilter, queue, NULL)) {
        g_printerr ("ERROR: Failed to link decodebin output chain.\n");
        goto error;
      }

      /*
       * Store callback state so the dynamic pad handler knows which element
       * should receive the first video/x-raw pad from qtiuridecodebin.
       */
      appctx->decodebin_link.target = capsfilter;
      appctx->decodebin_link.linked = FALSE;

      g_signal_connect (decodebin, "pad-added",
          G_CALLBACK (gst_app_decodebin_pad_added_cb),
          &appctx->decodebin_link);
    }
  }
 ...
 * it to display, encoded output, and metadata distribution.
 *
 * When no_output_overlay is FALSE a single qtivoverlay element sits before
 * the output tee, so all branches receive the annotated video. When TRUE,
 * qtivsplit separates three paths: the display branch gets its own overlay,
 * the video encode branch and the metadata branch receive clean frames.
 */
static gboolean
gst_app_create_output_pipe (GstAppContext * appctx, GstElement * user_tail)
{
  GstElement *pre_q = NULL;
  GstElement *overlay = NULL;
  GstElement *tee = NULL;
  GstElement *vsplit = NULL;
  GstPad *pad_display = NULL;
  GstPad *pad_video = NULL;
  GstPad *pad_meta = NULL;
  GstPad *sinkpad = NULL;

  /* Build the overlay + split topology first */
  ...
  if (!appctx->config.no_output_overlay) {
    ...
    if (!gst_element_link_many (user_tail, pre_q, overlay, tee, NULL)) {
      g_printerr ("ERROR: Failed to link overlay-enabled chain.\n");
      goto error;
    }
  } else {
    vsplit = gst_app_make_element ("qtivsplit", "output_split");

    if (!pre_q || !vsplit)
      goto error;

    gst_bin_add_many (GST_BIN (appctx->pipeline), pre_q, vsplit, NULL);

    if (!gst_element_link_many (user_tail, pre_q, vsplit, NULL)) {
      g_printerr ("ERROR: Failed to link overlay-disabled split chain.\n");
      goto error;
    }

    pad_display = gst_element_request_pad_simple (vsplit, "src_%u");
    pad_video   = gst_element_request_pad_simple (vsplit, "src_%u");
    pad_meta    = gst_element_request_pad_simple (vsplit, "src_%u");

    if (!pad_display || !pad_video || !pad_meta) {
      g_printerr ("ERROR: Failed to request vsplit pads.\n");
      goto error;
    }

    g_object_set (G_OBJECT (pad_video), "mode", "force-transform", NULL);
    g_object_set (G_OBJECT (pad_meta),  "mode", "force-transform", NULL);
  }

  /* Display branch */
  if (!appctx->config.no_display) {
    GstElement *display_q = gst_app_make_element ("queue", "display_queue");
    GstElement *display_overlay = NULL;
    GstElement *display_sink = gst_app_make_element ("waylandsink", "wayland_sink");
    gboolean ok = FALSE;

    if (!display_q || !display_sink)
      goto error;
    ...
    if (appctx->config.no_output_overlay) {
      gst_bin_add_many (GST_BIN (appctx->pipeline),
          display_q, display_overlay, display_sink, NULL);
    } else {
      gst_bin_add_many (GST_BIN (appctx->pipeline), display_q, display_sink, NULL);
    }

    if (appctx->config.no_output_overlay) {
      sinkpad = gst_element_get_static_pad (display_q, "sink");
      if (gst_pad_link (pad_display, sinkpad) != GST_PAD_LINK_OK) {
        g_printerr ("ERROR: Failed to link vsplit display pad.\n");
        gst_object_unref (sinkpad);
        goto error;
      }
      gst_object_unref (sinkpad);
      ok = gst_element_link_many (display_q, display_overlay, display_sink, NULL);
    } else {
      ok = gst_element_link_many (tee, display_q, display_sink, NULL);
    }

    if (!ok) {
      g_printerr ("ERROR: Failed to link display branch.\n");
      goto error;
    }
  }
  ...
  if (g_strcmp0 (appctx->config.output_type, "file") == 0 ||
      g_strcmp0 (appctx->config.output_type, "rtsp") == 0 ||
      g_strcmp0 (appctx->config.output_type, "webrtc") == 0) {
    GstElement *q = gst_app_make_element ("queue", "video_out_queue");
    GstElement *encoder = gst_app_create_h264_encoder ();
    GstElement *parse = gst_app_make_element ("h264parse", "video_parse");

    if (!q || !encoder || !parse)
      goto error;

    gst_bin_add_many (GST_BIN (appctx->pipeline), q, encoder, parse, NULL);

    if (appctx->config.no_output_overlay) {
      sinkpad = gst_element_get_static_pad (q, "sink");
      if (gst_pad_link (pad_video, sinkpad) != GST_PAD_LINK_OK) {
        g_printerr ("ERROR: Failed to link vsplit video pad.\n");
        gst_object_unref (sinkpad);
        goto error;
      }
      gst_object_unref (sinkpad);
      if (!gst_element_link_many (q, encoder, parse, NULL)) {
        g_printerr ("ERROR: Failed to link video output base chain.\n");
        goto error;
      }
    } else {
      if (!gst_element_link_many (tee, q, encoder, parse, NULL)) {
        g_printerr ("ERROR: Failed to link video output base chain.\n");
        goto error;
      }
    }
    ...
    if (g_strcmp0 (appctx->config.output_type, "file") == 0) {
       ...
       */
      GstElement *mux = gst_app_make_element ("mp4mux", "output_mp4_mux");
      GstElement *sink = gst_app_make_element ("filesink", "file_sink");
      ...
      gst_bin_add_many (GST_BIN (appctx->pipeline), mux, sink, NULL);
       ...
       */
      GstElement *sink = gst_app_make_element ("qtirtspbin", "rtsp_bin");
      ...
      gst_bin_add (GST_BIN (appctx->pipeline), sink);
       ...
       */
      GstElement *pay = gst_app_make_element ("rtph264pay", "webrtc_h264_pay");
      ...
      gst_bin_add_many (GST_BIN (appctx->pipeline), pay, appctx->webrtc, NULL);

      if (!gst_element_link (parse, pay)) {
  ...
  /* Metadata branch — always built */
  {
    GstElement *meta_q = gst_app_make_element ("queue", "meta_queue");
    GstElement *parser = gst_app_make_element ("qtimlmetaparser", "meta_parser");
    GstElement *meta_tee_q = gst_app_make_element ("queue", "meta_tee_queue");
    GstElement *meta_tee = gst_app_make_element ("tee", "meta_tee");
    GstElement *meta_out_q = gst_app_make_element ("queue", "meta_out_queue");

    if (!meta_q || !parser || !meta_tee_q || !meta_tee || !meta_out_q)
      goto error;

    gst_element_set_enum_property (parser, "module", "json");

    gst_bin_add_many (GST_BIN (appctx->pipeline),
        meta_q, parser, meta_tee_q, meta_tee, meta_out_q, NULL);

    if (appctx->config.no_output_overlay) {
      sinkpad = gst_element_get_static_pad (meta_q, "sink");
      if (gst_pad_link (pad_meta, sinkpad) != GST_PAD_LINK_OK) {
        g_printerr ("ERROR: Failed to link vsplit meta pad.\n");
        gst_object_unref (sinkpad);
        goto error;
      }
      gst_object_unref (sinkpad);

      if (!gst_element_link_many (meta_q, parser, meta_tee_q, meta_tee, meta_out_q, NULL)) {
        g_printerr ("ERROR: Failed to link metadata branch base chain.\n");
        goto error;
      }
    } else {
      if (!gst_element_link_many (tee, meta_q, parser, meta_tee_q, meta_tee, meta_out_q, NULL)) {
        g_printerr ("ERROR: Failed to link metadata branch base chain.\n");
        goto error;
      }
    }

    if (g_strcmp0 (appctx->config.output_type, "webrtc") == 0) {
      GstElement *appsink = gst_app_make_element ("appsink", "webrtc_meta_sink");
      ...
      gst_bin_add (GST_BIN (appctx->pipeline), appsink);

      if (!gst_element_link (meta_out_q, appsink)) {
        g_printerr ("ERROR: Failed to link WebRTC metadata output branch.\n");
        goto error;
      }
    } else if (g_strcmp0 (appctx->config.output_type, "rtsp") == 0) {
      GstElement *rtspbin =
          gst_bin_get_by_name (GST_BIN (appctx->pipeline), "rtsp_bin");

      if (!rtspbin)
        goto error;

      if (!gst_app_link_meta_queue_to_rtspbin (meta_out_q, rtspbin))
        goto error;

      gst_object_unref (rtspbin);
    } else {
      GstElement *filesink = gst_app_make_element ("filesink", "meta_file_sink");
      ...
      gst_bin_add (GST_BIN (appctx->pipeline), filesink);

      if (!gst_element_link (meta_out_q, filesink)) {
        g_printerr ("ERROR: Failed to link file metadata output branch.\n");
        goto error;
      }
    }

    if (appctx->config.enable_redis_meta) {
      GstElement *redis_q = gst_app_make_element ("queue", "redis_queue");
      GstElement *redis_sink = gst_app_make_element ("qtiredissink", "redis_sink");

      if (!redis_q || !redis_sink)
        goto error;

      g_object_set (G_OBJECT (redis_sink),
          "host", appctx->config.redis_host,
          "port", appctx->config.redis_port,
          "channel", appctx->config.redis_channel,
          "sync", FALSE,
          "async", FALSE,
          NULL);

      gst_bin_add_many (GST_BIN (appctx->pipeline), redis_q, redis_sink, NULL);

      if (!gst_element_link_many (meta_tee, redis_q, redis_sink, NULL)) {
        g_printerr ("ERROR: Failed to link Redis metadata branch.\n");
        goto error;
      }
    }
  }

  if (pad_display) gst_object_unref (pad_display);
  if (pad_video)   gst_object_unref (pad_video);
  if (pad_meta)    gst_object_unref (pad_meta);
 ...
 *   - user_tail is the last element from the user pipeline, returned to the
 *     caller so that gst_app_create_output_pipe() can link into it.
 *   - This function must create all demo-specific elements, add them to the
 *     pipeline, configure them, and link input_tail -> demo -> *user_tail.
 *
    ...
    GstElement ** user_tail)
{
  ...
  if (!qtimlvconverter  || !qtimltflite   || !qtimlpostprocess ||
      !postprocess_caps || !qtimetamux    || !qtiobjtracker) {
  ...
  /*
   * The detection model consumes one full frame per inference, so the
   * converter runs in non-cumulative image-batch mode. This is an enum
   * property and must be set through gst_element_set_enum_property(); passing
   * the nick as a plain string to g_object_set() would store a garbage
   * integer.
   */
  gst_element_set_enum_property (qtimlvconverter, "mode",
      "image-batch-non-cumulative");

  gchar *labels_path = g_strdup (appctx->config.labels_path);

  /*
   * Confidence is passed to qtimlpostprocess through the documented JSON
   * "settings" property rather than a separate settings file, because the
   * vehicle model needs only a threshold and no per-landmark configuration.
   */
  gchar *settings = g_strdup_printf ("{\"confidence\": %.1f}",
      appctx->config.confidence);

  g_object_set (G_OBJECT (qtimlpostprocess),
      "results", appctx->config.results,
      "labels", labels_path,
  ...
  gchar *model_path = g_strdup (appctx->config.model_path);
  ...
  if (!gst_element_link_many (input_tail, tee[0], queue[0], qtimetamux, NULL)) {
    g_printerr ("ERROR: Failed to link input to tee 1 / metamux.\n");
    return FALSE;
  }

  // Linking input to ML processing -> metamux
  if (!gst_element_link_many (tee[0], queue[1], qtimlvconverter, queue[2],
      qtimltflite, queue[3], qtimlpostprocess, postprocess_caps,
      queue[4], qtimetamux, NULL)) {
    g_printerr ("ERROR: Failed to link ML processing chain.\n");
    return FALSE;
  }

  // Linking metamux through the object tracker to user_tail
  if (!gst_element_link_many (qtimetamux, queue[5], qtiobjtracker, NULL)) {
    g_printerr ("ERROR: Failed to link metamux to object tracker.\n");
    return FALSE;
  }

  *user_tail = qtiobjtracker;
...
gst_app_create_pipe (GstAppContext * appctx)
{
  ...
  GstElement *user_tail = NULL;

  appctx->pipeline = gst_pipeline_new ("gst-vehicle-detection");
  ...
  if (!gst_app_create_user_pipe (appctx, input_tail, &user_tail)) {
    g_printerr ("ERROR: Failed to create user pipeline branch.\n");
    return FALSE;
  }

  if (!gst_app_create_output_pipe (appctx, user_tail)) {
...
main (int argc, char * argv[])
{
  ...
  appctx.config.confidence = DEFAULT_CONFIDENCE;
  appctx.config.results = DEFAULT_RESULTS;

  GOptionEntry entries[] = {
    { "input-type", 0, 0, G_OPTION_ARG_STRING, &appctx.config.input_type,
      "Input type: usb, isp, rtsp or file (file requires --input-config ending in .mp4 or .ts)", "TYPE" },
    ...
    { "no-display", 0, 0, G_OPTION_ARG_INT, &appctx.config.no_display,
      "Disable on-screen display: 1 or 0 (default: 0)", "1|0" },
    ...
    { "enable-redis-meta", 0, 0, G_OPTION_ARG_INT, &appctx.config.enable_redis_meta,
      "Enable Redis metadata output: 1 or 0 (default: 1)", "1|0" },
    { "model-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.model_path,
      "Path to TFLite model file", "PATH" },
    { "labels-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.labels_path,
      "Path to labels file", "PATH" },
    { "confidence", 0, 0, G_OPTION_ARG_DOUBLE, &appctx.config.confidence,
      "Detection confidence threshold, 0-100 (default: 51.0)", "THRESHOLD" },
    { "results", 0, 0, G_OPTION_ARG_INT, &appctx.config.results,
      "Maximum detections reported per frame (default: 30)", "COUNT" },
    { "redis-host", 0, 0, G_OPTION_ARG_STRING, &appctx.config.redis_host,
      "Redis host", "HOST" },
    { "redis-port", 0, 0, G_OPTION_ARG_INT, &appctx.config.redis_port,
      "Redis port", "PORT" },
    { "redis-channel", 0, 0, G_OPTION_ARG_STRING, &appctx.config.redis_channel,
      "Redis channel", "CHANNEL" },
    { NULL }
  };

  option_ctx = g_option_context_new ("- Runs a gstreamer vehicle detection "
