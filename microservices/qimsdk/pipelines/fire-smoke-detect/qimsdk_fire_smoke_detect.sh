...
qtimltflite name=stage_01_inference delegate=external external-delegate-path=libQnnTFLiteDelegate.so external-delegate-options="QNNExternalDelegate,backend_type=htp;" model=${MODEL_PATH_FIRE_SMOKE} \
qtimlpostprocess name=stage_01_postproc results=10 module=yolov8 labels=${LABELS_PATH_FIRE_SMOKE} settings="{\"confidence\": 20.0}" \
