 ...
 *   --no-display=1|0          (default: 0)
 *   --no-output-overlay=1|0   (default: 0)
 *   --enable-redis-meta=1|0   (default: 1)
 *
 * Example usage:
...
typedef struct GstAppConfig
{
  ...
  gchar *output_type;
  gchar *output_config;
  gchar *stage1_model_path;
  gchar *stage1_labels_path;
  gchar *stage1_settings_path;
  gchar *stage2_model_path;
  gchar *stage2_labels_path;
  gboolean no_display;
 ...
 */
static void
gst_app_decodebin_pad_added_cb (GstElement * decodebin,
                               GstPad * srcpad,
                               gpointer userdata)
{
  GstAppPadLinkData *link_data = (GstAppPadLinkData *) userdata;
...
gst_app_config_apply_defaults (GstAppConfig * config)
{
  ...
  if (config->stage1_model_path == NULL)
    config->stage1_model_path = g_strdup (STAGE1_MODEL_PATH);

  if (config->stage1_labels_path == NULL)
    config->stage1_labels_path = g_strdup (STAGE1_LABELS_PATH);

  if (config->stage1_settings_path == NULL)
    config->stage1_settings_path = g_strdup (STAGE1_SETTINGS_PATH);

  if (config->stage2_model_path == NULL)
    config->stage2_model_path = g_strdup (STAGE2_MODEL_PATH);

  if (config->stage2_labels_path == NULL)
    config->stage2_labels_path = g_strdup (STAGE2_LABELS_PATH);
...
gst_app_config_free (GstAppConfig * config)
{
  ...
  g_free (config->stage1_model_path);
  g_free (config->stage1_labels_path);
  g_free (config->stage1_settings_path);
  g_free (config->stage2_model_path);
  g_free (config->stage2_labels_path);
...
gst_app_create_input_pipe (GstAppContext * appctx, GstElement ** input_tail)
{
  ...
  if (g_strcmp0 (appctx->config.input_type, "usb") == 0) {
    ...
    */
    ...
    GstElement *decoder = NULL;
    GstElement *capsfilter_ts = NULL;
    GstCaps *ts_caps = NULL;

    filesrc = gst_app_make_element ("filesrc", "ts_filesrc");
    tsdemux = gst_app_make_element ("tsdemux", "ts_demux");
    parse   = gst_app_make_element ("h264parse", "ts_h264parse");
    decoder = gst_app_create_h264_decoder ();
    capsfilter_ts = gst_app_make_element ("capsfilter", "input_capsfilter");
    ...
    decodebin = gst_app_make_element ("qtiuridecodebin", "uri_decode");
    if (!decodebin || !queue)
      goto error;

    /*
    * Convert input_config to URI format.
    * - rtsp:// stays as-is
    ...
    */
    ...
    /*
    * Capsfilter ensures downstream ML gets expected format
    */
    capsfilter = gst_app_make_element ("capsfilter", "input_capsfilter");
    if (!capsfilter)
      goto error;
 ...
 *
 * user_tail is the last element of the user pipe.
 */
static gboolean
gst_app_create_output_pipe (GstAppContext * appctx, GstElement * user_tail)
{
  ...
  if (!appctx->config.no_output_overlay) {
    ...
    if (!gst_element_link_many (user_tail, pre_q, overlay, tee, NULL)) {
      g_printerr ("ERROR: Failed overlay-enabled chain.\n");
      goto error;
    }
    ...
    if (!gst_element_link_many (user_tail, pre_q, vsplit, NULL)) {
      g_printerr ("ERROR: Failed overlay-disabled split chain.\n");
      goto error;
    }
  ...
  if (g_strcmp0 (appctx->config.output_type, "file") == 0 ||
    ...
    if (appctx->config.no_output_overlay) {
      ...
      if (!gst_element_link_many (q, encoder, parse, NULL)) {
        g_printerr ("ERROR: Failed output branch base link.\n");
        goto error;
      }
    } else {
    ...
    if (g_strcmp0 (appctx->config.output_type, "file") == 0) {
      ...
      GstElement *mux = gst_app_make_element ("mp4mux", "mux");
      ...
      g_object_set (G_OBJECT (sink),
          "location", appctx->config.output_config,
          NULL);
  ...
  /* Metadata branch */
  {
    ...
    if (appctx->config.no_output_overlay) {
      ...
      if (!gst_element_link_many (
            meta_q, parser, meta_tee_q, meta_tee, meta_out_q, NULL)) {
        g_printerr ("ERROR: Failed metadata branch base link.\n");
        goto error;
      }
    } else {
      if (!gst_element_link_many (
            tee, meta_q, parser, meta_tee_q, meta_tee, meta_out_q, NULL)) {
        g_printerr ("ERROR: Failed metadata branch base link.\n");
        goto error;
      }
    }
    ...
    if (g_strcmp0 (appctx->config.output_type, "webrtc") == 0) {
      GstElement *appsink = gst_app_make_element ("appsink", "meta_sink");
      ...
      if (!gst_element_link (meta_out_q, appsink)) {
        g_printerr ("ERROR: Failed webrtc metadata branch link.\n");
        goto error;
      }
    } else if (g_strcmp0 (appctx->config.output_type, "rtsp") == 0) {
      GstElement *rtspbin =
      ...
      GstElement *filesink = gst_app_make_element ("filesink", "meta_file");
    ...
    if (appctx->config.enable_redis_meta) {
      ...
      gst_bin_add_many(GST_BIN(appctx->pipeline),
          redis_q, redis_sink, NULL);

      if (!gst_element_link_many(meta_tee, redis_q, redis_sink, NULL)) {
          g_printerr("ERROR: Failed Redis metadata branch\n");
          goto error;
      }
    }
  }
 ...
 *   - user_tail is the last element from the user pipeline.
    ...
    GstElement ** user_tail)
{
  ...
  gchar *stage1_labels = g_strdup (appctx->config.stage1_labels_path);

  gchar *stage1_settings = g_strdup (appctx->config.stage1_settings_path);
  ...
  gchar *stage2_labels = g_strdup (appctx->config.stage2_labels_path);
  ...
  gchar *stage1_model = g_strdup (appctx->config.stage1_model_path);
  ...
  gchar *stage2_model = g_strdup (appctx->config.stage2_model_path);
  ...
  if (!gst_element_link_many (input_tail, tee[0], queue[0], qtimetamux_stage1,
      NULL)) {
    g_printerr ("ERROR: Failed to link input to tee 1 / metamux 1.\n");
    return FALSE;
  }

  // Linking input to first stage of ML processing -> metamux 1
  if (!gst_element_link_many (tee[0], queue[1], qtimlvconverter_stage1, queue[2],
      qtimltflite_stage1, queue[3], qtimlpostprocess_stage1, postprocess_caps_stage1,
      queue[4], qtimetamux_stage1, NULL)) {
    g_printerr ("ERROR: Failed to link stage 1 ML processing chain.\n");
    return FALSE;
  }

  // Linking metamux 1 through object tracker to tee 2
  if (!gst_element_link_many (qtimetamux_stage1, queue[5], qtiobjtracker,
      tee[1], NULL)) {
    g_printerr ("ERROR: Failed to link metamux 1 to object tracker / tee 2.\n");
    return FALSE;
  }

  // Linking tee 2 to metamux 2
  if (!gst_element_link_many (tee[1], queue[6], qtimetamux_stage2, NULL)) {
    g_printerr ("ERROR: Failed to link tee 2 to metamux 2.\n");
    return FALSE;
  }

  // Linking input to second stage of ML processing -> metamux 2
  if (!gst_element_link_many (tee[1], queue[7], qtimlvconverter_stage2, queue[8],
      qtimltflite_stage2, queue[9], qtimlpostprocess_stage2, postprocess_caps_stage2,
      queue[10], qtimetamux_stage2, NULL)) {
    g_printerr ("ERROR: Failed to link stage 2 ML processing chain.\n");
    return FALSE;
  }

  *user_tail = qtimetamux_stage2;
...
main (int argc, char * argv[])
{
  ...
  GOptionEntry entries[] = {
    { "input-type", 0, 0, G_OPTION_ARG_STRING, &appctx.config.input_type,
      "Input type: usb, isp, rtsp, mp4 or ts", "TYPE" },
    ...
    { "stage1-model-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.stage1_model_path,
      "Path to stage1 model file", "PATH" },
    { "stage1-labels-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.stage1_labels_path,
      "Path to stage1 labels file", "PATH" },
    { "stage1-settings-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.stage1_settings_path,
      "Path to stage1 settings file", "PATH" },
    { "stage2-model-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.stage2_model_path,
      "Path to stage2 model file", "PATH" },
    { "stage2-labels-path", 0, 0, G_OPTION_ARG_STRING, &appctx.config.stage2_labels_path,
      "Path to stage2 labels file", "PATH" },
