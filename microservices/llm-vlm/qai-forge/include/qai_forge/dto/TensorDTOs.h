...
/**
 * Non-owning input pieces for one backend invocation. The scheduler can join
 * several requests without first allocating and copying a combined tensor.
 * Sources must remain alive until infer() returns.
 */
struct TensorInputSegment {
    const uint8_t* data = nullptr;
    size_t         size = 0;
};

struct SegmentedInputTensor {
    std::string                     name;
    std::vector<int64_t>            shape;
    TensorDataType                  dtype;
    std::vector<TensorInputSegment> segments;
    size_t                          byte_size = 0;
    uint8_t                         padding_value = 0;
};

struct SegmentedTensorInferenceRequest {
    std::string                       model;
    std::string                       request_id;
    std::vector<SegmentedInputTensor> inputs;
