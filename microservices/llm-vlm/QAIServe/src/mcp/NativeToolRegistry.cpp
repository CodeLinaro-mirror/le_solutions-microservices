...
void NativeToolRegistry::registerTool(std::unique_ptr<ITool> tool) {
    if (!tool) { std::cerr << "[NativeToolRegistry] null tool\n"; return; }
    std::lock_guard<std::mutex> lock(mutex_);
    const std::string tool_name = tool->name();
    if (tools_.count(tool_name))
        std::cerr << "[NativeToolRegistry] WARNING: overwriting '" << tool_name << "'\n";
...
bool NativeToolRegistry::hasTools() const { return !tools_.empty(); }
size_t NativeToolRegistry::toolCount() const { return tools_.size(); }
bool NativeToolRegistry::hasTool(const std::string& name) const { return tools_.count(name) > 0; }

McpToolResult NativeToolRegistry::callTool(const std::string& name, const json& arguments) {
    auto it = tools_.find(name);
    if (it == tools_.end()) {
        std::cerr << "[NativeToolRegistry] Unknown tool '" << name << "'\n";
        return ITool::makeErrorResult("Native tool '" + name + "' is not registered.");
    }
    try {
