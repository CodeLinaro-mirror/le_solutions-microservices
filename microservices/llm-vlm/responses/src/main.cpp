...
int main() {
    ...
    // Child processes (genai-inference-worker, MCP servers) will inherit these privileges.
    qai_forge::utils::drop_privileges_and_bind_devices();

    // Step 2: Read port from environment variable (default 9002)
    const char* port_env = std::getenv("RESPONSES_PORT");
    int port = port_env ? std::stoi(port_env) : 9002;

    std::cout << "[main] Starting GenAI Responses API server on port " << port << std::endl;
    ...
    drogon::app()
        ...
        .registerBeginningAdvice([]() {
            ...
            //   1. RESPONSES_MCP_CONFIG env var (explicit path)
            //   2. ./mcp_servers.json (same directory as binary)
            //   3. /etc/qai-responses/mcp_servers.json (system-wide config)
            auto& mcp_registry = McpClientRegistry::getInstance();

            // Always register the "native" virtual server so McpClientRegistry
            // can route "native__<tool>" calls to McpNativeClient → NativeToolRegistry.
            // This must happen AFTER native tools are registered above.
            {
            ...
            const char* mcp_config_env = std::getenv("RESPONSES_MCP_CONFIG");
            ...
            if (mcp_registry.serverCount() > 0) {
                ...
                std::cout << "[main] No MCP servers configured "
                          << "(set RESPONSES_MCP_CONFIG or create ./mcp_servers.json "
                          << "to enable MCP tool support)." << std::endl;
            }

            std::cout << "[main] Responses API server started." << std::endl;
        })
        .run();
