...
FILE NAME
    cloud_connect_kafka_logging.c

DESCRIPTION
    Implementation file for the logging function of Cloud Connect KAFKA service.
*/
#include <stdio.h>
#include <time.h>
#include <sys/time.h>
...
void log_printf(LogLevel level, const char *filename, int line, const char *format, ...)
{
    ...
    printf("[CCK %s, %s, %s:%d] - ", get_timestamp(), get_log_level(level), filename, line);
