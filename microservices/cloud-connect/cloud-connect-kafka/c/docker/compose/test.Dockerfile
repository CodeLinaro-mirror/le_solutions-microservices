...
# Install all the dependent packages to compile cloud connect kafka application.
...
RUN mkdir -p kafka_ut_coverage
RUN mkdir -p kafka_ut_report

CMD ["/bin/sh", "-c", \
    "rm -rf kafka_ut_coverage/*; \
    rm -rf kafka_ut_report/*; \
    ./test/run_tests -ojunit; \
    cp *.xml kafka_ut_report; \
    rm -rf build_ut/*test*; rm -rf build_ut/*mock*; rm -rf build_ut/*main*; \
    gcovr -r . --html --html-details -o kafka_ut_coverage/index.html --gcov-ignore-parse-errors; \
    cp -r build_ut kafka_ut_coverage; \
    chmod 777 -R kafka_ut_report; \
    chmod 777 -R kafka_ut_coverage;"]
