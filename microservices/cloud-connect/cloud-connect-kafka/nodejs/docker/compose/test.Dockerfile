...
COPY --chown=node:node ./src/__mocks__ ./__mocks__/
COPY --chown=node:node ./src/__test__ ./__test__/
COPY --chown=node:node ./src/constants ./constants/
COPY --chown=node:node ./src/utils ./utils/
COPY --chown=node:node ./src/.c8rc.json ./
COPY --chown=node:node ./src/babel.config.json ./
COPY --chown=node:node ./src/cloud-connect.js ./
COPY --chown=node:node ./src/index.js ./
COPY --chown=node:node ./src/jest.config.js ./
