...
async function insertFATrigger(data) {
    ...
    try {
        ...
        const res = await conn.query(`INSERT INTO fire_analytics_triggers ` +
            `(monitor_id, trigger_id, trigger_name, params)` +
...
async function getAllFATriggers() {
    let conn;
    try {
        conn = await fetchConn();
        const res = await conn.query(`SELECT * from fire_analytics_triggers;`);
        return res;
    } catch (e) {
        throw e;
    } finally {
...
async function getFATriggersByMonitor(data) {
    let conn;
    try {
        conn = await fetchConn();
        // Escape apostrophes
        ...
        const res = await conn.query(`SELECT * from fire_analytics_triggers where monitor_id = '${data}';`);
        return res;
    } catch (e) {
        throw e;
    } finally {
...
async function getFATrigger(data) {
    let conn;
    try {
        conn = await fetchConn();
        // Escape apostrophes
        ...
        const res = await conn.query(`SELECT * from fire_analytics_triggers where trigger_id = '${data}';`);
        return res;
    } catch (e) {
        throw e;
    } finally {
...
async function insertFAAlert(jsonData) {
    let conn;
    try {
        ...
        conn = await fetchConn();
        const res = await conn.query(`INSERT INTO fire_analytics_alerts ` +
...
async function getAlerts(data) {
    ...
    try {
        ...
        let rows = await conn.query(`select fa.source_trigger_id, UNIX_TIMESTAMP(fa.time) as time, fa.causes, ft.* FROM fire_analytics_alerts fa` +
            ` inner join fire_analytics_triggers ft on fa.source_trigger_id = ft.trigger_id ` +
...
async function removeTrigger(data) {
    ...
    try {
        ...
        const res = await conn.query(`DELETE FROM fire_analytics_triggers WHERE ` +
            `trigger_id = '${data}';`);
        return res;
    } catch (e) {
        throw e;
...
async function createTriggersTable() {
    ...
    try {
        ...
        const res = await conn.query(`CREATE TABLE IF NOT EXISTS fire_analytics_triggers ` +
...
async function createAlertsTable() {
    ...
    try {
        ...
        const res = await conn.query('CREATE TABLE IF NOT EXISTS fire_analytics_alerts ' +
            '(source_trigger_id VARCHAR(255) not null, ' +
            'time datetime(6), causes text, ' +
            'CONSTRAINT `fa_trigger_alerts_fk` ' +
            'FOREIGN KEY (source_trigger_id) REFERENCES fire_analytics_triggers (trigger_id) ' +
            'on delete cascade ' +
            'on update RESTRICT );');
        return res;
    } catch (e) {
