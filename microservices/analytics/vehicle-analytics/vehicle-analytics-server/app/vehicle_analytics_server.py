...
from vehicle_analytics import VehicleAnalytics, ANALYTICS_CHANNEL, DETECTION_CHANNEL_PREFIX, REGION_KEY, TRIGGER_KEY
...
async def register_pubsub_listener(r: redis.Redis, va: VehicleAnalytics):
    ...
    def detection_message_handler(message):
        ...
        if message is not None:
            va.enqueue_message(now, message)
    ...
    def analytics_request_handler(message):
        ...
        if message and message['type'] == 'message':
            ...
            # {
            #     'sync_id': sync_id,
            #     'monitor_id': data.monitorId, OR 'regionId': data.regionId
            #     'from_time': data.fromTime,
            #     'to_time': data.toTime,
            #     'analytics_type': analyticsType, # either 'count' or 'heatmap'
            # }
            ...
            try:
                ...
                if( request.analytics_type == 'count'):
                    ...
                    if region_id:
                        asyncio.create_task(va.run_count_query(r, request.sync_id, None, request.region_id, request.from_time, request.to_time))
                    elif monitor_id:
                        asyncio.create_task(va.run_count_query(r, request.sync_id, request.monitor_id, None, request.from_time, request.to_time))
                elif( request.analytics_type == 'heatmap'):
                    asyncio.create_task(va.run_heatmap_query(r, request.sync_id, request.monitor_id, request.from_time, request.to_time))
                else:
    ...
    # --- Region Keyspace Handler ---
    def region_update_handler(message):
        logger.info(f'Subscribed to region updates on key: {REGION_KEY}')
        if message['type'] == 'pmessage':
            logger.info(f'Regions update detected: {message}')
            asyncio.create_task(va.update_regions())

    # --- Trigger Keyspace Handler ---
    def trigger_update_handler(message):
        logger.info(f'Subscribed to trigger updates on key: {TRIGGER_KEY}')
        if message['type'] == 'pmessage':
            logger.info(f'Triggers update detected: {message}')
            asyncio.create_task(va.update_triggers())
    ...
    await pubsub.psubscribe(**{'__keyspace@0__:' + REGION_KEY: region_update_handler})
...
async def async_main():
    ...
    try:
        ...
        pubsub = await register_pubsub_listener(r, va)
        loop = asyncio.get_event_loop()
        loop.add_signal_handler(signal.SIGTERM, quit_handler)

        await va.init()
        statistics_process_task = va.start_statistics_task()
