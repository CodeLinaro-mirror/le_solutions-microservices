...
module.exports.getAnalyticsCount = async function getAnalyticsCount (req, res, next, body) {
    ...
    try {
        ...
        */
        ...
        // Make sure there is a region associated with the request
        let region = await db.getRARegion(req.query.regionId);
        if (region.length !== 1) {
            res.status(400).json({
                reason: 'Error: RegionId entered not found. Please create a Region first or check spelling.'
            });
        }
