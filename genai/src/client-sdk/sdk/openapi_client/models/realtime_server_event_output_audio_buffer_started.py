...
class RealtimeServerEventOutputAudioBufferStarted(BaseModel):
    ...
    **WebRTC Only:** Emitted when the server begins streaming audio to the client. This event is emitted after an audio content part has been added (`response.content_part.added`) to the response. [Learn more](/docs/guides/realtime-model-capabilities#client-and-server-events-for-audio-in-webrtc).
    ...
    type: StrictStr = Field(description="The event type, must be `output_audio_buffer.started`.")
    response_id: StrictStr = Field(description="The unique ID of the response that produced the audio.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['output_audio_buffer.started']):
            raise ValueError("must be one of enum values ('output_audio_buffer.started')")
        return value
