...
class RealtimeClientEventOutputAudioBufferClear(BaseModel):
    ...
    **WebRTC Only:** Emit to cut off the current audio response. This will trigger the server to stop generating audio and emit a `output_audio_buffer.cleared` event. This  event should be preceded by a `response.cancel` client event to stop the  generation of the current response. [Learn more](/docs/guides/realtime-model-capabilities#client-and-server-events-for-audio-in-webrtc).
    """ # noqa: E501
    event_id: Optional[StrictStr] = Field(default=None, description="The unique ID of the client event used for error handling.")
    type: StrictStr = Field(description="The event type, must be `output_audio_buffer.clear`.")
    __properties: ClassVar[List[str]] = ["event_id", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['output_audio_buffer.clear']):
            raise ValueError("must be one of enum values ('output_audio_buffer.clear')")
        return value
